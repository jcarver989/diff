    ...
    pub fn catalog() -> Arc<[Self]> {
        ReviewTheme::catalog()
            ...
            .filter_map(|descriptor| {
                ReviewTheme::builtin(&descriptor.id)
                    .ok()
                    .map(|theme| Self::new(descriptor.name, theme))
            })
            .collect()
    }
}
...
impl ThemeSelection {
    ...
    pub fn new(current: &ReviewTheme, themes: Arc<[ThemeChoice]>) -> Option<Self> {
        ...
        let selected = themes
            .iter()
            .position(|choice| choice.theme.id() == current.id())
            .unwrap_or(0);
        Some(Self {
            themes,
            selected,
    ...
    pub fn selected_theme(&self) -> ReviewTheme {
        self.themes[self.selected].theme.clone()
    }
