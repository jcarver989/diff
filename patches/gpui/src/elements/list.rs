...
struct StateInner {
    ...
    scrollbar_drag_scroll_fraction: Option<f32>,
...
impl ListState {
    ...
    pub fn new(item_count: usize, alignment: ListAlignment, overdraw: Pixels) -> Self {
        let this = Self(Rc::new(RefCell::new(StateInner {
            ...
            scrollbar_drag_scroll_fraction: None,
    ...
    pub fn reset(&self, element_count: usize) {
        let old_count = {
            ...
            state.uniform_item_height = None;
            state.pending_scroll = None;
            state.scrollbar_drag_start_height = None;
            state.scrollbar_drag_scroll_fraction = None;
    ...
    pub fn scrollbar_drag_started(&self) {
        let mut state = self.0.borrow_mut();
        let scroll_max = state.max_scroll_offset();
        let scroll_offset = state.scroll_top(&state.logical_scroll_top());
        state.scrollbar_drag_scroll_fraction = Some(if scroll_max > Pixels::ZERO {
            (scroll_offset / scroll_max).clamp(0.0, 1.0)
        } else {
            0.0
        });
    ...
    ///
    /// Reconciles the frozen drag range with the live content height so the thumb remains at the
    /// position where the user released it, even if rows were measured during the drag.
    pub fn scrollbar_drag_ended(&self) {
        let mut state = self.0.borrow_mut();
        if state.scrollbar_drag_start_height.take().is_none() {
            return;
        }
        let Some(scroll_fraction) = state.scrollbar_drag_scroll_fraction.take() else {
            return;
        };
        let live_scroll_max = state.max_scroll_offset();
        state.set_offset_from_scrollbar(point(
            Pixels::ZERO,
            -live_scroll_max * scroll_fraction,
        ));
    }
...
impl StateInner {
    ...
    fn set_offset_from_scrollbar(&mut self, point: Point<Pixels>) {
        ...
        if let Some(scroll_fraction) = self.scrollbar_drag_scroll_fraction.as_mut() {
            *scroll_fraction = if scroll_max > Pixels::ZERO {
                (new_scroll_top / scroll_max).clamp(0.0, 1.0)
            } else {
                0.0
            };
        }
...
impl Element for List {
    ...
    fn prepaint(
        ...
        // If the width changes, measured heights are stale. Retain the configured uniform
        // fallback so unmeasured rows continue contributing to content and prefix heights while
        // visible rows converge to their actual sizes at the new width.
        if state
            .last_layout_bounds
...
mod test {
    ...
    #[gpui::test]
    fn test_uniform_height_hint_survives_width_invalidation(cx: &mut TestAppContext) {
        let cx = cx.add_empty_window();
        let state = ListState::new(100, crate::ListAlignment::Top, px(0.))
            .with_uniform_item_height(px(32.));
        ...
        assert_eq!(state.max_offset_for_scrollbar().y, px(3072.));
    ...
    #[gpui::test]
    fn test_scrollbar_drag_release_preserves_fraction_after_height_growth(
        ...
        item_height.set(550);
        state.remeasure_items(3..4);
        cx.draw(point(px(0.), px(0.)), size(px(100.), px(200.)), |_, _| {
            view.into_any_element()
        });

        assert_eq!(state.max_offset_for_scrollbar().y, px(300.));
        assert_eq!(state.scroll_px_offset_for_scrollbar().y, px(-150.));
