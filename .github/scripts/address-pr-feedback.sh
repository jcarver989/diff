...
pr_number=$(jq -er '.issue.number // .pull_request.number' "$GITHUB_EVENT_PATH")
...
jq -n \
  --arg task "Address all comments and review feedback on pull request #$pr_number." \
  --argjson number "$pr_number" \
  --argjson issueComments "$issue_comments" \
  --argjson reviewComments "$review_comments" \
  --argjson reviews "$reviews" \
  '{kind: "pull_request_feedback", task: $task, pullRequest: {number: $number}, issueComments: $issueComments, reviewComments: $reviewComments, reviews: $reviews}' \
  >"$task_file"
